        ...
        <> {pokemon ?
            <article className={`card-poke border-${pokemon?.types[0].type.name}`}>
                <header className={`card-poke__header bg-${pokemon?.types[0].type.name}`}>
                    <img className='card-poke__sprite' src={pokemon?.sprites.other['official-artwork'].front_default} alt="" onClick={handleClick}/>
                </header>
                <section className="card-poke__body">
                    <h3 className={`card-poke__name letter-${pokemon?.types[0].type.name}`} onClick={handleClick}>{pokemon?.name} </h3>
